...
def controller(query=st.session_state.query):
    ...
    while not st.session_state.done and i > 0:
        ...
        # Here is the content of that first message; print for troublehooting
        first_answer = message["content"]
        ...
        if message.get("function_call"):
            ...
            st.session_state.message_history.append(message2)
            second_answer = message2["content"]
            st.write(f'Here is the second answer: {second_answer}')
...
def calculate_expression(expression: str) -> float:
    """
    Processes an expression generated by a user's question.
    If a variable is required, use x and provide the expression as follows. Do not invoke solve(), simply provide the arguments to solve:
    Example 1: "x**2 + 2*x + 1, x"  
    Example 2: "6 *3 +4"
    
    The variable is optional; use if required to answer the user's question.
    Cosine is cos (not math.cos) and pi is pi.
    For trig, use radians. (radians = degrees * pi/180).
   
    :param expression: A expression that will be submitted to sympify solve function as an argument
    :type expression: string
    :return: A float representing the processed result of the expression
    :rtype: float
    """
...
def start_chat(query):
    ...
    if st.button('Go', key = "starter"):
        ...
        1. Use the 'calculate_expression' function to solve an expression. The expression syntax must work as a parameter to the sympy solve function. For trig, use radians. (radians = degrees * pi/180). When your full response is complete, include: ```Now we are done.``` 
        2. Use the 'search_internet' function to search the internet for an answer. When your full response is complete, include: ```Now we are done.``` 
...
#     if st.button('Go'):
#         query = """ You have access to two functions to assist responses:
#         1. Use the 'calculate_expression' function to calculate any expression. Provide an expression, e.g, '5*6' that the function will solve using sympy.solve('expression'). For trig, use radians. (radians = degrees * pi/180). When your answer is complete, always include ```Now we are done.``` to indicate you are finished.
